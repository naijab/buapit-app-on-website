...
<title><?= $web_title; ?> : จัดการไฟล์ดาวน์โหลด</title>
</head>
...
<tbody>
  <?php
        $stmt = $db_con->prepare("SELECT * FROM buapit_download WHERE download_by = {$row['user_school_id']} &&  download_by_user = {$row['user_id']} ORDER BY download_id DESC");
        $stmt->execute();
        ...
        ?>
            ...
            <td><a href="<?php echo $row['download_url']; ?>"><?php echo $row['download_url']; ?></a></td>
            <td>
            <a id="edit-btn" class="btn btn-warning waves-effect waves-float" href="edit_download?eid=<?php echo $row['download_id']; ?>" >
            แก้ไข</a>
            <a id="<?php echo $row['news_id']; ?>" class="btn btn-danger waves-effect waves-float" href="javascript:delete_id(<?php echo $row['download_id']; ?>)">
            ลบ
                  </a></td>
            </tr>
          <?php } ?>
              ...
              <!--Add form-->
                <form action="lib/add_download" enctype="multipart/form-data" method="post" >
                  <div class="row">
                    <div class="col-md-12">
        ...
        function delete_id(id)
            {
                if(confirm('ต้องการลบใช่หรือไม่ ?'))
                 {
                 window.location.href='lib/delete_download?did='+id;
                 }
            }
    </script>
